...
# Open data
fc <- nc_open('FossilPollen/Data/msb-paleon-2/2Kyrs_Comp_SD_Level2_v1.0.nc')

# Get lon, lat, time
# x and y are flipped to get hte right easting and northing directions
y <- ncvar_get(fc, 'x')
x <- ncvar_get(fc, 'y')
time <- ncvar_get(fc, 'Time')
...
# Add lat & long to full dataframe
full_melt_sd <- full_melt_sd |>
  dplyr::select(-c(x, y)) |>
  mutate(long = space_latlong$y, lat = space_latlong$x)

# Check to make sure the points match up with expectation
states <- map_data('state', region = c('michigan', 'minnesota', 'wisconsin'))

full_melt_sd |>
  ggplot(aes(x = long, y = lat, color = hemlock)) +
...
# Remove empty grid cells
for_removal <- c()
for(i in 1:nrow(full_melt_sd)){
  if(is.na(full_melt_sd$ash[i]) & is.na(full_melt_sd$beech[i]) & is.na(full_melt_sd$birch[i]) &
     is.na(full_melt_sd$elm[i]) & is.na(full_melt_sd$hemlock[i]) & is.na(full_melt_sd$maple[i]) &
     is.na(full_melt_sd$oak[i]) & is.na(full_melt_sd$other_conifer[i]) & is.na(full_melt_sd$other_hardwood[i]) &
     is.na(full_melt_sd$pine[i]) & is.na(full_melt_sd$spruce[i]) & is.na(full_melt_sd$tamarack[i])){
    for_removal <- c(for_removal, i)
  }
}

full_melt_sd <- full_melt_sd[-for_removal,]
...
# Remove dates for longer ago than we have climate reconstructions
full_melt_sd <- full_melt_sd |>
  # convert from thousands of years before 1950 to years before 1950
  mutate(time = time * 100)
...
save(full_melt_sd, file = 'FossilPollen/Data/full_melt_sd.RData')
